...
function LecturesPage ({ match: { params: { code }}}) {
  ...
  // Load lectures list
  useEffect(() => {
    fetch(`http://localhost:8000/${code}/lectures`).then(r => r.json()).then(data => {
        setLectures(data)
    })
  }, [setLectures, code]) 
  ...
  const handleSubmit = e => { 
    ...
    if (searchTerm === '') {
      ...
      fetch(`http://localhost:8000/${code}/lectures/search/${searchTerm.toLowerCase()}`).then(r => r.json()).then(data => {
        setLectures(data)
      })
    }
  }
  ...
  const handleAddLectureSubmit = (formData) => {
    fetch(`http://localhost:8000/${code}/lectures`, {
        ...
        if (r.status === 200) {
            fetch(`http://localhost:8000/${code}/lectures`).then(r => r.json()).then(data => setLectures(data))
        } 
    })
  }
