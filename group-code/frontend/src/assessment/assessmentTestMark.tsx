...
export default function AssessmentTestMark() {
    ...
    useEffect(() => {
        ...
        AssessmentService.renderAssessmentAttemptMark(para)
            .then(res => {
                ...
                for (let index = 0; index < res.data.length; index++) {
                    ...
                    question["problemDescription"] = res.data[index].Question.question_description
                    question["type"] = res.data[index].Question.type
                    if (res.data[index].type != "Essay") {
                        question["choice"] = JSON.parse(res.data[index].Question.choices).choices
                    }
                    question["answerAttempt"] = JSON.parse(res.data[index].Question.answer_attempt).answer_attempt

                    if (JSON.parse(res.data[index].Question.answer).answer == "") {
                        question["answer"] = [""]
                    }
                    else {
                        question["answer"] = JSON.parse(res.data[index].Question.answer).answer
                    }
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    if (regFloat.test(mark) == false) {
                                                        ...
                                                        AssessmentService.updateAttemptMark(para)
                                                            .then((res) => {
                                                                console.log(res.data)
