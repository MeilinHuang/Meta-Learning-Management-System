...
import { ar } from 'date-fns/locale'; import AssessmentEditTestModal from './components/assessmentEditTestModal';
                  ...
                  {problem[0].problemDescription === "" ? <></> :
                    <>
                      {problem.map((prob) => (
                        <div
                          key={prob.questionID}
                          className={`group flex items-center px-2 py-2 text-sm font-medium rounded-md ${prob.questionID == isActive.toString() ? 'text-white bg-indigo-500' : ''}`}
                        >
                          <button
